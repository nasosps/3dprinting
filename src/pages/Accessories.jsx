...
export default function Accessories() {
  ...
  const [sheet, setSheet] = useState(null) // null | { mode: 'add'|'edit', item? }
  ...
  return (
    <div className="p-4 pb-24">
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-xl font-semibold text-white">Αξεσουάρ</h1>
        ...
        {accessories.map(a => (
          <div key={a.id} className="bg-[#1a1a1f] rounded-xl p-4 border border-[#2e2e38] flex items-center justify-between">
            <div>
              ...
              {a.cost != null && <div className="text-xs text-gray-500 mt-0.5">{Number(a.cost).toFixed(2)}€ / τεμ.</div>}
            </div>
            <div className="flex items-center gap-3">
              {a.stock != null && (
                <div className={`text-sm font-semibold ${a.stock <= 2 ? 'text-orange-400' : 'text-white'}`}>×{a.stock}</div>
              )}
              <button onClick={() => openEdit(a)} className="text-gray-400 active:text-white p-1"><Pencil size={16} /></button>
              <button onClick={() => setDelConfirm(a)} className="text-gray-400 active:text-red-400 p-1"><Trash2 size={16} /></button>
            </div>
          </div>
        ))}
      </div>
      ...
      <BottomSheet open={!!delConfirm} onClose={() => setDelConfirm(null)} title="Διαγραφή;">
        <p className="text-gray-300 mb-6">Διαγραφή <strong className="text-white">{delConfirm?.name}</strong>; Δεν μπορεί να αναιρεθεί.</p>
