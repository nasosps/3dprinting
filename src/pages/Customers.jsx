  ...
  return (
    <div className="p-4 pb-24">
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-xl font-semibold text-white">Πελάτες</h1>
            ...
            <div className="flex items-start justify-between">
              <div>
                ...
                <div className="flex flex-col gap-1 mt-1">
                  {c.phone && <a href={`tel:${c.phone}`} className="flex items-center gap-2 text-sm text-gray-400"><Phone size={13} />{c.phone}</a>}
                  {c.email && <a href={`mailto:${c.email}`} className="flex items-center gap-2 text-sm text-gray-400"><Mail size={13} />{c.email}</a>}
                </div>
              </div>
              <div className="flex items-center gap-2">
                <button onClick={() => openEdit(c)} className="text-gray-400 active:text-white p-1"><Pencil size={16} /></button>
                <button onClick={() => setDelConfirm(c)} className="text-gray-400 active:text-red-400 p-1"><Trash2 size={16} /></button>
              </div>
            </div>
          </div>
        ))}
      ...
      <BottomSheet open={!!delConfirm} onClose={() => setDelConfirm(null)} title="Διαγραφή;">
        <p className="text-gray-300 mb-6">Διαγραφή <strong className="text-white">{delConfirm?.name}</strong>;</p>
